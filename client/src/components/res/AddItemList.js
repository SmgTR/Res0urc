...
const AddItemList = ({ back, id }) => {
  ...
  useEffect(() => {
    if (updateLink) {
      ...
      const link = current[0].links.find((link) => link._id === selected[0].id);

      setItem({ ...link });

      // const update

      checkImage(link.image, (exists) => {
        if (exists) {
          setPreview(link.image);
        } else {
    ...
    data.map((key) => {
      ...
      if (key in item) {
        metaData.map((meta) => {
          ...
          if (key === 'video' && meta['video:secure_url'] !== undefined) {
  ...
  const onSubmit = (e) => {
    const links = current[0].links.filter((link) => link._id !== item._id);

    !updateLink
      ? addListItem({ ...item }, false)
      : addListItem({ ...item }, links);
    back();
  };
      ...
      <form onSubmit={setDefault} className='add__item-form'>
        ...
        {fetched && (
          <Fragment>
            ...
            <button className='add'>{updateLink ? 'Update' : 'Add'}</button>
          </Fragment>
        )}
      </form>
    </div>
